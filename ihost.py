...
# --- Coin System / Brand Configuration ---
BOT_BRAND_NAME = "𝐁𝐊 𝐇𝐎𝐒𝐓𝐈𝐍𝐆"  # yeha tumhar brand naam dalo
BRAND_TAGLINE = "🚀 Fast • Reliable • 24/7 Bot Hosting"
FREE_START_COINS = 50          # notun user start korlay ki koyta free coin pabe
COIN_COST_PER_HOUR = 1         # 1 coin = 1 ghonta hosting
LOW_COIN_ALERT_THRESHOLD = 5   # eyer niche gele low-coin warning jabe
COIN_BILL_INTERVAL_SECONDS = 3600  # protti ghontay coin kata hobe
...
def load_data():
    ...
    try:
        ...
        # Load coin balances
        c.execute('SELECT user_id, balance FROM coins')
        for coin_user_id, coin_balance in c.fetchall():
            user_coins[coin_user_id] = coin_balance

        conn.close()
        logger.info(f"Data loaded: {len(active_users)} users, {len(user_subscriptions)} subscriptions, {len(admin_ids)} admins, {len(user_coins)} coin wallets.")
    except Exception as e:
...
def run_script(script_path, script_owner_id, user_folder, file_name, message_obj_for_reply, attempt=1):
    ...
    # ================= COIN CHECK =================
    if attempt == 1 and script_owner_id != OWNER_ID and script_owner_id not in admin_ids:
        current_coins = get_user_coins(script_owner_id)
        if current_coins < COIN_COST_PER_HOUR:
            bot.reply_to(
                message_obj_for_reply,
                f"❌ Insufficient coins to start '{file_name}'.\n"
                f"🪙 Balance: {int(current_coins)} | Required: {COIN_COST_PER_HOUR} coin/hour\n\n"
                f"👉 Contact admin to recharge your coins.",
...
# --- Coin System Database Operations ---
def get_user_coins(user_id):
    """Return a user's coin balance. Owner/Admins are unlimited."""
    if user_id == OWNER_ID or user_id in admin_ids:
        return float('inf')
    return user_coins.get(user_id, 0)

def set_user_coins_db(user_id, balance):
    """Persist an exact coin balance for a user (never below 0)."""
    balance = max(0, int(balance))
    with DB_LOCK:
        conn = sqlite3.connect(DATABASE_PATH, check_same_thread=False)
        c = conn.cursor()
        try:
            c.execute('INSERT OR REPLACE INTO coins (user_id, balance) VALUES (?, ?)', (user_id, balance))
            conn.commit()
            user_coins[user_id] = balance
            logger.info(f"Coin balance for {user_id} set to {balance}")
        except sqlite3.Error as e: logger.error(f"❌ SQLite error setting coins for {user_id}: {e}")
        except Exception as e: logger.error(f"❌ Unexpected error setting coins for {user_id}: {e}", exc_info=True)
        finally: conn.close()
    return balance

def add_user_coins(user_id, amount):
    """Add (or subtract, with a negative amount) coins for a user."""
    new_balance = user_coins.get(user_id, 0) + amount
    return set_user_coins_db(user_id, new_balance)

def deduct_user_coins(user_id, amount):
    """Deduct coins for a user, floored at 0. Returns the new balance."""
    new_balance = user_coins.get(user_id, 0) - amount
    return set_user_coins_db(user_id, new_balance)
...
def create_main_menu_inline(user_id):
    ...
    if user_id in admin_ids:
        ...
        markup.add(admin_buttons[6])  # Coins
...
def create_coins_menu():
    markup = types.InlineKeyboardMarkup(row_width=2)
    markup.row(
        types.InlineKeyboardButton('➕ Add Coins', callback_data='add_coins'),
        types.InlineKeyboardButton('➖ Remove Coins', callback_data='remove_coins')
    )
    markup.row(types.InlineKeyboardButton('🔍 Check Coins', callback_data='check_coins'))
    markup.row(types.InlineKeyboardButton('🔙 Back to Main', callback_data='back_to_main'))
    return markup

def get_admin_contact_markup():
    markup = types.InlineKeyboardMarkup()
    markup.add(types.InlineKeyboardButton('📞 Contact Admin', url=f'https://t.me/{YOUR_USERNAME.replace("@", "")}'))
    return markup

def notify_low_coins(user_id, chat_id):
    """Warn a user their coin balance is running low."""
    try:
        balance = get_user_coins(user_id)
        bot.send_message(
            chat_id,
            f"⚠️ 𝐋𝐨𝐰 𝐂𝐨𝐢𝐧 𝐁𝐚𝐥𝐚𝐧𝐜𝐞!\n\n🪙 Remaining: {balance} coin(s)\n"
            f"⏳ Your running bot(s) will stop once coins run out.\n\n"
            f"👉 Contact admin to recharge and keep your bots alive.",
            reply_markup=get_admin_contact_markup()
        )
    except Exception as e:
        logger.error(f"Failed to send low-coin alert to {user_id}: {e}")
...
# 🔒 Force Join Check
    ...
    if is_new_user:
        ...
        if user_id != OWNER_ID and user_id not in admin_ids and user_id not in user_coins:
            add_user_coins(user_id, FREE_START_COINS)
            logger.info(f"Granted {FREE_START_COINS} free start coins to new user {user_id}")
        try:
    ...
    coin_balance = get_user_coins(user_id)
    coin_str = "Unlimited ♾️" if coin_balance == float('inf') else f"{int(coin_balance)} 🪙"
    bonus_line = f"\n🎁 You received {FREE_START_COINS} free coins for joining!" if is_new_user and coin_balance != float('inf') else ""

    welcome_msg_text = (f"✨ {BOT_BRAND_NAME} ✨\n{BRAND_TAGLINE}\n\n"
                        ...
                        f"🪙 Coin Balance: {coin_str}{bonus_line}\n\n"
                        f"🤖 Host & run Python (`.py`) or JS (`.js`) scripts.\n"
                        f"   Upload single scripts or `.zip` archives.\n"
                        f"⏱️ Hosting cost: {COIN_COST_PER_HOUR} coin = 1 hour of runtime.\n\n"
...
def _logic_coins_panel(message):
    if message.from_user.id not in admin_ids:
        bot.reply_to(message, "⚠️ Admin permissions required.")
        return
    bot.reply_to(message, "🪙 Coin Management\nUse inline buttons from /start or admin command menu.", reply_markup=create_coins_menu())
...
def handle_callbacks(call):
    ...
    try:
        ...
        elif data == 'coins': admin_required_callback(call, coins_management_callback)
        elif data == 'add_coins': admin_required_callback(call, add_coins_init_callback)
        elif data == 'remove_coins': admin_required_callback(call, remove_coins_init_callback)
        elif data == 'check_coins': admin_required_callback(call, check_coins_init_callback)
        else:
...
def coins_management_callback(call):
    bot.answer_callback_query(call.id)
    try:
        bot.edit_message_text("🪙 Coin Management\nSelect action:",
                              call.message.chat.id, call.message.message_id, reply_markup=create_coins_menu())
    except Exception as e: logger.error(f"Error showing coins menu: {e}")

def add_coins_init_callback(call):
    bot.answer_callback_query(call.id)
    msg = bot.send_message(call.message.chat.id, "🪙 Enter User ID & amount to add (e.g., `12345678 50`).\n/cancel to abort.")
    bot.register_next_step_handler(msg, process_add_coins_details)

def process_add_coins_details(message):
    admin_id_check = message.from_user.id
    if admin_id_check not in admin_ids: bot.reply_to(message, "⚠️ Not authorized."); return
    if message.text.lower() == '/cancel': bot.reply_to(message, "Coin add cancelled."); return
    try:
        parts = message.text.split()
        if len(parts) != 2: raise ValueError("Incorrect format")
        coin_user_id = int(parts[0].strip()); amount = int(parts[1].strip())
        if coin_user_id <= 0 or amount <= 0: raise ValueError("User ID/amount must be positive")

        new_balance = add_user_coins(coin_user_id, amount)
        logger.info(f"{amount} coins added for {coin_user_id} by admin {admin_id_check}. New balance: {new_balance}")
        bot.reply_to(message, f"✅ Added {amount} 🪙 to `{coin_user_id}`.\nNew balance: {new_balance} 🪙")
        try: bot.send_message(coin_user_id, f"🎉 {amount} coins added to your wallet!\n🪙 New balance: {new_balance} coins.")
        except Exception as e: logger.error(f"Failed to notify {coin_user_id} of new coins: {e}")
    except ValueError as e:
        bot.reply_to(message, f"⚠️ Invalid: {e}. Format: `ID amount` or /cancel.")
        msg = bot.send_message(message.chat.id, "🪙 Enter User ID & amount, or /cancel.")
        bot.register_next_step_handler(msg, process_add_coins_details)
    except Exception as e: logger.error(f"Error processing add coins: {e}", exc_info=True); bot.reply_to(message, "Error.")

def remove_coins_init_callback(call):
    bot.answer_callback_query(call.id)
    msg = bot.send_message(call.message.chat.id, "🪙 Enter User ID & amount to remove (e.g., `12345678 20`).\n/cancel to abort.")
    bot.register_next_step_handler(msg, process_remove_coins_details)

def process_remove_coins_details(message):
    admin_id_check = message.from_user.id
    if admin_id_check not in admin_ids: bot.reply_to(message, "⚠️ Not authorized."); return
    if message.text.lower() == '/cancel': bot.reply_to(message, "Coin removal cancelled."); return
    try:
        parts = message.text.split()
        if len(parts) != 2: raise ValueError("Incorrect format")
        coin_user_id = int(parts[0].strip()); amount = int(parts[1].strip())
        ...
        new_balance = deduct_user_coins(coin_user_id, amount)
        logger.info(f"{amount} coins removed for {coin_user_id} by admin {admin_id_check}. New balance: {new_balance}")
        bot.reply_to(message, f"✅ Removed {amount} 🪙 from `{coin_user_id}`.\nNew balance: {new_balance} 🪙")
        try: bot.send_message(coin_user_id, f"ℹ️ {amount} coins were deducted from your wallet by admin.\n🪙 New balance: {new_balance} coins.")
        except Exception as e: logger.error(f"Failed to notify {coin_user_id} of coin removal: {e}")
    except ValueError as e:
        bot.reply_to(message, f"⚠️ Invalid: {e}. Format: `ID amount` or /cancel.")
        msg = bot.send_message(message.chat.id, "🪙 Enter User ID & amount to remove, or /cancel.")
        bot.register_next_step_handler(msg, process_remove_coins_details)
    except Exception as e: logger.error(f"Error processing remove coins: {e}", exc_info=True); bot.reply_to(message, "Error.")

def check_coins_init_callback(call):
    bot.answer_callback_query(call.id)
    msg = bot.send_message(call.message.chat.id, "🪙 Enter User ID to check coin balance.\n/cancel to abort.")
    bot.register_next_step_handler(msg, process_check_coins_id)

def process_check_coins_id(message):
    admin_id_check = message.from_user.id
    if admin_id_check not in admin_ids: bot.reply_to(message, "⚠️ Not authorized."); return
    if message.text.lower() == '/cancel': bot.reply_to(message, "Coin check cancelled."); return
    try:
        coin_user_id_check = int(message.text.strip())
        if coin_user_id_check <= 0: raise ValueError("ID must be positive")
        balance = get_user_coins(coin_user_id_check)
        balance_str = "Unlimited ♾️" if balance == float('inf') else f"{int(balance)} 🪙"
        bot.reply_to(message, f"ℹ️ User `{coin_user_id_check}` coin balance: {balance_str}")
    except ValueError:
        ...
        bot.register_next_step_handler(msg, process_check_coins_id)
    except Exception as e: logger.error(f"Error processing check coins: {e}", exc_info=True); bot.reply_to(message, "Error.")
...
# --- Coin Billing Loop (hourly hosting charges) ---
def coin_billing_loop():
    """Background loop: every hour, charge running bots 1 coin/hour.
    Stops any bot whose owner runs out of coins and tells them to contact admin."""
    logger.info(f"🪙 Coin billing loop started. Charging {COIN_COST_PER_HOUR} coin every {COIN_BILL_INTERVAL_SECONDS}s per running bot.")
    while True:
        time.sleep(COIN_BILL_INTERVAL_SECONDS)
        try:
            for script_key in list(bot_scripts.keys()):
                ...
                if owner_id == OWNER_ID or owner_id in admin_ids:
                    continue  # Owner/Admins host for free

                if not is_bot_running(owner_id, file_name):
                    continue

                new_balance = deduct_user_coins(owner_id, COIN_COST_PER_HOUR)
                logger.info(f"⏱️ Charged {COIN_COST_PER_HOUR} coin to {owner_id} for '{file_name}'. New balance: {new_balance}")

                if new_balance <= 0:
                    logger.warning(f"🛑 {owner_id} ran out of coins. Stopping '{file_name}'.")
                    current_info = bot_scripts.get(script_key)
                    if current_info:
                        kill_process_tree(current_info)
                        if script_key in bot_scripts:
                            del bot_scripts[script_key]
                    try:
                        bot.send_message(
                            chat_id,
                            f"🛑 '{file_name}' has been stopped — you're out of coins (🪙 0).\n\n"
                            f"👉 Contact admin to recharge your coins and restart your bot.",
                            reply_markup=get_admin_contact_markup()
                        )
                    except Exception as e:
                        logger.error(f"Failed to notify {owner_id} about coin-exhaustion stop: {e}")
                elif new_balance <= LOW_COIN_ALERT_THRESHOLD:
                    notify_low_coins(owner_id, chat_id)
        except Exception as e:
            logger.error(f"Error in coin billing loop: {e}", exc_info=True)
...
if __name__ == '__main__':
    ...
    keep_alive()
    threading.Thread(target=coin_billing_loop, daemon=True).start()
